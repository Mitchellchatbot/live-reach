...
const VisitorInfoSidebar = ({
  ...
  return <div className={cn("border-l border-border/30 hidden lg:flex flex-col transition-all duration-200 glass", isOpen ? "w-64" : "w-10")}>
      <Collapsible open={isOpen} onOpenChange={setIsOpen} className="flex flex-col h-full">
        {/* Toggle button */}
        <CollapsibleTrigger asChild>
          <Button variant="ghost" size="sm" className={cn("h-8 w-full justify-center border-b border-border rounded-none", isOpen && "justify-end px-2")}>
        ...
        <CollapsibleContent className="flex-1 overflow-y-auto bg-sidebar">
          ...
          {hasTreatmentInfo && <div className="p-3 border-t border-border space-y-1">
          ...
          {assignedAgent && <div className="p-3 border-t border-border">
