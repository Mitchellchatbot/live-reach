...
import { Check, Phone, Mail, MapPin, Menu, X, Star, ChevronRight, Send } from 'lucide-react';
...
const PRIMARY_DARK = '#0A5C5F';
const ACCENT = '#D4A853';
const DARK_BG = '#1A2F2F';
...
const Comparison2 = () => {
  ...
  return (
    <div className="min-h-screen" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 md:h-20">
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold tracking-tight" style={{ color: DARK_BG }}>
              Treatment Center
            </span>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <a key={link} href="#" className="text-sm font-medium transition-colors hover:opacity-70" style={{ color: DARK_BG }}>
                {link}
              </a>
            ))}
          </div>
          ...
          <button className="lg:hidden" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            {mobileMenuOpen ? <X className="h-6 w-6" style={{ color: DARK_BG }} /> : <Menu className="h-6 w-6" style={{ color: DARK_BG }} />}
          </button>
        </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-t px-4 py-4 space-y-3" style={{ backgroundColor: 'white', borderColor: '#e5e7eb' }}>
            {navLinks.map((link) => (
              <a key={link} href="#" className="block text-sm font-medium py-2" style={{ color: DARK_BG }}>
                {link}
              </a>
            ))}
            <a
      ...
      >
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, rgba(13,115,119,0.8) 0%, rgba(26,47,47,0.7) 100%)` }} />
      ...
      {/* ─── Insurance Logos ─── */}
      <section className="py-16" style={{ backgroundColor: '#F0F7F7' }}>
            ...
            <ul className="space-y-4">
              {features.map((f) => (
                <li key={f} className="flex items-start gap-3">
                  <div className="h-6 w-6 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ backgroundColor: '#E0F2F1' }}>
      ...
      {/* ─── Testimonials ─── */}
      <section className="py-20" style={{ backgroundColor: '#F0F7F7' }}>
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {testimonials.map((t) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full flex items-center justify-center text-white font-bold text-sm" style={{ backgroundColor: PRIMARY }}>
                    {t.name.charAt(0)}
                  </div>
      ...
      <footer className="py-16" style={{ backgroundColor: DARK_BG }}>
        ...
        <div className="max-w-2xl mx-auto px-4 mt-14">
          <div className="rounded-2xl p-8" style={{ backgroundColor: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)' }}>
            <h3 className="text-xl font-bold text-center mb-2" style={{ color: 'white' }}>
              Get Confidential Help Today
            </h3>
            <p className="text-xs text-center mb-6" style={{ color: 'rgba(255,255,255,0.6)' }}>
              Fill out the form below and a caring counselor will reach out to you shortly.
            </p>

            {formSubmitted ? (
              <div className="text-center py-6">
                <div className="h-12 w-12 rounded-full flex items-center justify-center mx-auto mb-3" style={{ backgroundColor: PRIMARY }}>
                  <Check className="h-6 w-6 text-white" />
                </div>
                <p className="text-sm font-semibold" style={{ color: 'white' }}>Thank you! We'll be in touch soon.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-lg text-white font-bold text-sm transition-transform hover:scale-[1.02]"
                  style={{ backgroundColor: ACCENT }}
                >
